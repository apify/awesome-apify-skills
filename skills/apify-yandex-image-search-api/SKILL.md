...
1. Fetch the current input schema with `apify actors info "johnvc/Scrape-Yandex" --input --user-agent apify-awesome-skills/apify-yandex-image-search-api 2>/dev/null`, then build the query. `text` is required. Set `include_image_search` true; turn `include_organic_results` false unless you also want the web SERP in the same run.
...
5. Read the completed run’s `defaultDatasetId` into `DATASET_ID`, then fetch its rows with `apify datasets get-items "$DATASET_ID" --format json --user-agent apify-awesome-skills/apify-yandex-image-search-api 2>/dev/null`. Then filter the dataset to `item_type` = "image_search", and read each item's `image_results` array (`original` is the full-size URL, `link` the hosting page). Download files only when requested and where the hosting page’s license allows it; validate the target and redirects as public HTTP(S) URLs before fetching.
