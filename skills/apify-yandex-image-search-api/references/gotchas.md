...
Pricing model: pay per event. Read the current `pricingInfos` from `apify api GET /v2/acts/johnvc~Scrape-Yandex -H 'User-Agent: apify-awesome-skills/apify-yandex-image-search-api'` before estimating. Use the latest pricing entry whose `startedAt` is not in the future and the tier of the authenticated account. `apify actors info --json` can truncate this Actor's metadata, so verify that the response parses before using it.
