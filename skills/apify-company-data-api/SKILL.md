...
2. A bare slug works as input. `ignite-visibility` and `https://clutch.co/profile/ignite-visibility` mean the same thing.
...
5. Map `name`, `website`, `headquarters`, `employees`, and `min_project_size` straight onto CRM fields. Every row is flat JSON.
6. Check `result_type` before treating a row as a company. An `error` row carries `error_message` and `error_type`.
7. `maxItems` is the hard spend ceiling for the whole run, reviews included.
